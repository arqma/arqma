...
used by Arqma. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Arqma's
...
## Header
This header is sent for every Arqma p2p message.
...
Arqma when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Arqma specific
