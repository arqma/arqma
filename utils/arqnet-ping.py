...
if len(sys.argv) < 3:
    ...
    if m:
        ...
        if "result" in r and "service_node_x25519_privkey" in r["result"]:
            ...
            print("Loaded x25519 keys (pub: {}) from arqmad @ {}".format(hexstr(x_key.public_key), sys.argv[1]))
        else:
            x_key = PrivateKey.generate()
            print("Generated x25519 key {} (arqmad @ {} did not return SN privkeys)".format(hexstr(x_key.public_key), sys.argv[1]))
    else:
        print("Error: {} does not look like a valid arqmad RPC host:port value".format(sys.argv[1]), sys.stderr)
        badargs = True
...
if badargs:
    print("\nUsage: {} localhost:19994 PUBKEY -- pings PUBKEY's arqnet.\n\n"
...
for s in states:
    ...
    if pk in missed:
        missed.remove(pk)
    ip, port = s["public_ip"], s["arqnet_port"]
    if not ip or not port:
        print("SN {} has no IP/arqnet port: {}:{}".format(pk, ip, port))
