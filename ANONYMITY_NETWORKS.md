# Anonymity Networks with Galaxia

Currently only Tor and I2P have been integrated into Galaxia. The usage of
...
is made or until `galaxiad` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `galaxia-wallet-cli` and
`galaxia-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--proxy` which tells `galaxiad` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--proxy tor,127.0.0.1:9050,10`
> `--proxy i2p,127.0.0.1:9000`

which tells `galaxiad` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `galaxiad` the inbound address, network
type, and max connections:

> `--anonymous-inbound rveahdfho7wo4b2m.onion:29996,127.0.0.1:29996,25`
> `--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000,127.0.0.1:30000`

which tells `galaxiad` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:29996" and forwarded to `galaxiad`
...
forwarded to `galaxiad` localhost port 30000.
...
`galaxiad` RPC port - which is independent from the configuration for incoming
...
> HiddenServiceDir /var/lib/tor/data/galaxia
...
This will store key information in `/var/lib/tor/data/galaxia` and will forward
"Tor port" 29996 to port 29996 of ip 127.0.0.1. The file
`/usr/lib/tor/data/galaxia/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Galaxia-Network should have
similar timestamps.
...
An ISP can passively monitor `galaxiad` connections from a node and observe when
...
only using I2P for broadcasting Galaxia transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of an Galaxia
user.
...
### Intermittent Galaxia Syncing

If a user only runs `galaxiad` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `galaxiad` as often as possible to conceal when transactions are being sent.
